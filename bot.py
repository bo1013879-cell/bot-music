...
# Quay lại dùng Spotify API chính thống để tránh lỗi DRM
import spotipy
...
YTDL_OPTIONS = {
    ...
    "cookiefile": "cookies.txt", # Dùng thêm dòng này nếu bạn đã tạo file cookies.txt
    "http_headers": {
        "User-Agent": (
...
class MusicCog(commands.Cog):
    ...
    # === DÙNG SPOTIFY API CHÍNH THỐNG ĐỂ LẤY TÊN BÀI HÁT (KHÔNG BỊ DRM) ===
    def _get_spotify_tracks(self, url):
        if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET:
            raise ValueError("Vui lòng cấu hình SPOTIFY_CLIENT_ID và SPOTIFY_CLIENT_SECRET")

        cm = SpotifyClientCredentials(client_id=SPOTIFY_CLIENT_ID, client_secret=SPOTIFY_CLIENT_SECRET)
        sp = spotipy.Spotify(client_credentials_manager=cm)
        track_names = []

        # Lấy bài hát đơn lẻ (Track)
        track_match = re.match(r"https?://open\.spotify\.com/track/([a-zA-Z0-9]+)", url)
        if track_match:
        ...
        if playlist_match:
            ...
            while results:
                for item in results['items']:
                    if item['track']:
                        track_names.append(f"{item['track']['name']} {item['track']['artists'][0]['name']}")
                if results['next']:
        ...
        if album_match:
            ...
            while results:
                for item in results['items']:
                    track_names.append(f"{item['name']} {artist_name}")
                if results['next']:
                    results = sp.next(results)
                else:
                    break
        ...
        raise ValueError("Link Spotify không hợp lệ.")

    @app_commands.command(name="play", description="Phat nhac tu link hoac ten bai (hỗ trợ Spotify)")
    @app_commands.describe(query="Link YouTube, Spotify hoac ten bai hat")
    async def play(self, interaction: discord.Interaction, query: str):
        ...
        # === XỬ LÝ SPOTIFY BẰNG SPOTIPY (Lấy tên -> Tìm trên YouTube) ===
        if "open.spotify.com" in query:
            await interaction.followup.send("🔄 Đang phân tích danh sách từ Spotify...")
            try:
                track_names = self._get_spotify_tracks(query)
                await interaction.followup.send(f"✅ Tìm thấy **{len(track_names)}** bài hát, đang tìm kiếm trên YouTube...")
                
                for track_name in track_names:
                    try:
                        # Dùng ytsearch để tìm bài hát trên YouTube
                        sources = await YTDLSource.from_url(f"ytsearch:{track_name}", loop=self.bot.loop)
                        if sources:
                            all_sources.extend(sources)
                    except Exception as e:
                        print(f"[WARN] Không tìm thấy bài: {track_name} - {e}")
                
                if not all_sources:
                    return await interaction.followup.send(embed=discord.Embed(description="❌ Không tìm thấy bài hát nào từ Spotify trên YouTube.", color=0xFF4444))
            
            except spotipy.exceptions.SpotifyException as e:
                if e.http_status == 401:
                    return await interaction.followup.send(embed=discord.Embed(description="❌ **Lỗi 401 Spotify API**: Bạn cần vào [Spotify Developer Dashboard](https://developer.spotify.com/dashboard), nhấn nút **Move to Production**, chờ vài giờ, rồi Reset Client Secret và cập nhật lại trên Railway.", color=0xFF4444))
                else:
                    return await interaction.followup.send(embed=discord.Embed(description=f"Lỗi Spotify API: {e}", color=0xFF4444))
            except Exception as e:
                return await interaction.followup.send(embed=discord.Embed(description=f"Lỗi xử lý Spotify: {e}", color=0xFF4444))

        else:
            # === XỬ LÝ YOUTUBE / SEARCH THƯỜNG ===
            if not re.match(r"https?://", query):
                query = f"ytsearch:{query}"
            try:
                all_sources = await YTDLSource.from_url(query, loop=self.bot.loop)
        ...
        # Nếu đang phát, thông báo đã thêm vào queue
        added = all_sources[0]
        embed = discord.Embed(
        ...
        if len(all_sources) > 1:
            embed.add_field(name="Playlist/Album", value=f"Added {len(all_sources)} tracks", inline=False)
        if added.thumbnail:
