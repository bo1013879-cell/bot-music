...
class MusicCog(commands.Cog):
    ...
    # === HÀM MỚI: Lấy danh sách tên bài hát từ link Spotify ===
    def _get_spotify_track_names(self, url):
        """Trả về list tên bài hát (dạng 'Tên bài - Nghệ sĩ') từ link Spotify."""
        if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET:
            raise ValueError("Vui lòng cấu hình SPOTIFY_CLIENT_ID và SPOTIFY_CLIENT_SECRET trong .env")
        ...
        if album_match:
            ...
            while results['next']:
                results = sp.next(results)
                items.extend(results['items'])
            # Cần lấy tên album và nghệ sĩ chính để tìm chính xác hơn
            album_info = sp.album(album_id)
            album_artist = album_info['artists'][0]['name']
            for item in items:
                name = item['name']
                track_names.append(f"{name} {album_artist}")
            return track_names

        raise ValueError("Link Spotify không hợp lệ hoặc chưa được hỗ trợ.")
    ...
    async def play(self, interaction: discord.Interaction, query: str):
        ...
        if "open.spotify.com" in query:
            try:
                track_names = self._get_spotify_track_names(query)
                await interaction.followup.send(
                    embed=discord.Embed(
                        description=f"🔄 Đang tìm kiếm **{len(track_names)}** bài hát từ Spotify trên YouTube...",
                        color=EMBED_COLOR
                    )
                )
                for track_name in track_names:
                    try:
                        # Tìm trên YouTube với ytsearch:
                ...
                if not all_sources:
                    return await interaction.followup.send(
                        embed=discord.Embed(description="❌ Không tìm thấy bài hát nào từ Spotify trên YouTube.", color=0xFF4444)
                    )
            except Exception as e:
                return await interaction.followup.send(
                    embed=discord.Embed(description=f"Lỗi xử lý Spotify: {e}", color=0xFF4444)
                )
        else:
        ...
        # Nếu đang không phát gì, bắt đầu phát
