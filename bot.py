...
YTDL_OPTIONS = {
    ...
    "noplaylist": False,  # Cho phép lấy danh sách phát
    ...
    "cookiefile": "cookies.txt", # Thêm dòng này nếu bạn dùng cookies.txt để tránh lỗi bot YouTube
    "http_headers": {
        "User-Agent": (
...
class MusicCog(commands.Cog):
    ...
    async def play(self, interaction: discord.Interaction, query: str):
        ...
        # === XỬ LÝ SPOTIFY MỚI (Dùng yt-dlp parse trực tiếp) ===
        if "open.spotify.com" in query:
            await interaction.followup.send("🔄 Đang tìm kiếm danh sách phát từ Spotify trên YouTube...")
            try:
                # Dùng yt-dlp tự phân tích link Spotify (Không cần Token, không bị lỗi 401)
                loop = self.bot.loop
                data = await loop.run_in_executor(
                    None, lambda: YTDLSource.ytdl.extract_info(query, download=False)
                )
                
                if data and 'entries' in data:
                    # Duyệt qua toàn bộ playlist yt-dlp tìm được
                    for entry in data['entries']:
                        if entry:
                            all_sources.append(YTDLSource._make(entry))
                elif data:
                    all_sources.append(YTDLSource._make(data))
